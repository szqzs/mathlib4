...
/-!
# Shared Simplex Algorithm Functions

This module contains simplex algorithm functions that are shared between the linarith oracle
and the linear optimization tactics (maximize/minimize). These functions implement the core
pivot operations and variable selection logic using Bland's rule.

The shared functions include:
- `doPivotOperation`: Core pivot operation implementation
- `chooseEnteringVar`: Bland's rule for selecting entering variable
- `chooseExitingVar`: Bland's rule for selecting exiting variable  
- `choosePivots`: Combined pivot selection

These functions were consolidated to eliminate code duplication and ensure consistent
algorithmic behavior across different uses of the simplex method.
-/
...
/--
Given indexes `exitIdx` and `enterIdx` of exiting and entering variables in the `basic` and
`free` arrays, performs pivot operation, i.e. expresses one through the other and makes the free
one basic and vice versa.
-/
